...
$leaves = $conn->prepare("SELECT * FROM `leaves` WHERE `status` = 'approve' ORDER BY `leaves`.`form_date` DESC");
$leaves->execute();
                ...
                if($leave['formdate_session'] ==  $leave['enddate_session']){
                  ...
                  echo '
                    <tr id="row_'.$leave['id'].'">
                      ...
                      if($roleId == 1){
                        echo '<td> <a class="btn btn-danger" onclick="cancelLeave('.$leave['id'].')">Cancel</a></td>
                          ';
                      }
