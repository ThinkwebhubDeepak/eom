<?php 
    ...
    $preparation = $conn->prepare("SELECT * FROM `projectefficiency` WHERE `user_id` = ? AND `type` = 'preparation' AND `status` = 'start'");
    $preparation->execute([$_SESSION['userId']]);
...
<div class="pre-loader <?php echo !($preparation) ? 'hidden' : '' ?>">
    <button class="btn btn-warning complete-btn" onclick="completePreparation()">Complete Preparation</button>
    ...
    <div class="drone">
        <p class="loadingText">Preparation . . .</p>
...
<main style="margin-top: 100px;" class="<?php echo ($preparation) ? 'hidden' : '' ?>">
    <div class="container-flude pt-5" div class="container-flude pt-5">
        <div class="container-flude px-5">
            <div class="d-flex justify-content-between " style="padding: 0 0 40px 0;">
                <p class="fw-bold page_heading">Preparation Task</p>
                <div style="display: flex; height:40px;">
                    ...
                    <button style="margin:0 20px;width:300px" onclick="projectPreparation() " class="btn btn-warning">Satrt Preparation</button>
                </div>
            </div>
        </div>
    </div>
                        ...
                        <div class="row form-row mb-3">
                            <div class="col-12 col-sm-6 p-2">
                                <div class="form-group">
                                    ...
                                    <input type="hidden" id="project_id" name="project_id" value="<?php echo $preparation['project_id'] ?>" required>
                                    <input type="hidden" id="project_id" name="task_id" value="<?php echo $preparation['type'] ?>" required>
                                </div>
                            </div>
    ...
    function completePreparation() {
      $.ajax({
        ...
        data: {
          ...
          project_id: <?php echo $preparation['project_id'] ?? 0 ?>
        },
        dataType: 'json',
        success: function(response) {
          console.log(response);
