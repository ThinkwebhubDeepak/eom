...
$page_name = 'resign-task';
include 'includes/header.php';

$sql = $conn->prepare("SELECT * FROM `tasks` Where `status` = 'assign_pro'");
$sql->execute();
...
$vectorCount = $conn->prepare("SELECT * FROM `tasks` Where `vector_status` = 'assign_vector'");
$vectorCount->execute();
$vectorCount = $vectorCount->fetchAll(PDO::FETCH_ASSOC);
$countVector = count($vectorCount);

$sql2 = $conn->prepare("SELECT * FROM `users` ORDER BY `first_name` ASC");
$sql2->execute();
    ...
    <table id="dataTable" class="display">
      ...
      <tbody id="tableBody">
        ...
        foreach ($tasks as $task) {
          ...
          $assign = $conn->prepare("SELECT * FROM `assign` WHERE `task_id` = ? AND `role` = 'pro' AND `status` = 'assign'");
...
<script>
  ...
  $('#addAssign').submit(function (event) {
    ...
    $.ajax({
      url: 'includes/settings/api/reassignApi.php',
      type: 'POST',
      data: formData,
      processData: false,
      contentType: false,
