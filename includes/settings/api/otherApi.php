<?php
    ...
    if (($_SERVER['REQUEST_METHOD'] == 'GET') && ($_GET['type'] == 'getLastLog')) {
        ...
        if ($_GET['task_id'] != '' && $_GET['project_id']  != ''  && $_GET['status']  != '' ) {
            $sql = $conn->prepare("SELECT * FROM `work_log` WHERE `task_id` = ? AND `project_id` = ? AND `next_status` = ? ORDER BY `id` DESC");
            $sql->execute([$_GET['task_id'],$_GET['project_id'] , $_GET['status']]);
            $data = $sql->fetch(PDO::FETCH_ASSOC);
            if ($data) {
                $givenTimestamp = strtotime($data['updated_at']);
                $currentTimestamp = time();
                $timeDifferenceInSeconds = $currentTimestamp - $givenTimestamp;
                $total_minutes = floor($timeDifferenceInSeconds / 60);
                ...
                if($checkBreak['total_time']){
                    $minutes = $total_minutes - $checkBreak['total_time'];
                }else{
