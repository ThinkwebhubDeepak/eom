<?php 
  ...
  if($roleId == 1){
    ...
    $proTask->execute();
    $tasks = $proTask->fetchAll(PDO::FETCH_ASSOC);
    ...
    $vetorCount->execute();
    $vetorCount = $vetorCount->fetchAll(PDO::FETCH_ASSOC);
    $countVector = count($vetorCount);
    ...
    $assign = $conn->prepare('SELECT * FROM `assignproject` WHERE ( `user_id` = ? OR `assign_by` = ? ) AND `access` = 1  ORDER BY `project_id` DESC');
    ...
    foreach ($assign as $value) {
      if($tempdata != $value['project_id']){
        ...
        $proTask = $conn->prepare("SELECT * FROM `tasks` Where `status` = 'pending' AND `project_id` = ?");
        $proTask->execute([$value['project_id']]);
        $tasks[] = $proTask->fetchAll(PDO::FETCH_ASSOC);
        
        $qcTask = $conn->prepare("SELECT * FROM `tasks` Where `status` = 'ready_for_qc' AND `project_id` = ?");
        $qcTask->execute([$value['project_id']]);
        $qcCount[] = $qcTask->fetchAll(PDO::FETCH_ASSOC);
        
        $qaTask = $conn->prepare("SELECT * FROM `tasks` Where `status` = 'ready_for_qa' AND `project_id` = ?");
        $qaTask->execute([$value['project_id']]);
        $qaCount[] = $qaTask->fetchAll(PDO::FETCH_ASSOC);
   
        $vetorTask = $conn->prepare("SELECT * FROM `tasks` Where `vector_status` = 'pending' AND `project_id` = ?");
        $vetorTask->execute([$value['project_id']]);
        $vetorCount[] = $vetorTask->fetchAll(PDO::FETCH_ASSOC);
      }
    }
    ...
    $vetorCount = array_merge(...$vetorCount);
      ...
      <div class="modal-body">
        <div class="card-body p-2">
          <form id="addAssign">
            ...
            <input type="hidden" class="form-control" name="role" value="pro" required>
...
<main style="margin-top: 100px;">
  <div class="btn-group   justify-content-center d-flex  mt-3 " role="group">
    <a href="#" style="display: flex;align-items: center;margin: 0 10px">
      <button type="button" class="btn btn-danger position-relative btn_active">
        Pro
    ...
    <a href="assign-vector.php" style="display: flex;align-items: center;margin: 0 10px">
      <button type="button" class="btn btn-primary position-relative">
        Vector
        <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">
  ...
  <div class="container-flude pt-5 p-2">
    <div class="container-flude px-5">
      <div class="d-flex justify-content-between" style="font-size: 25px;">
        <p class="fw-bold page_heading">Assign Pro</p>
