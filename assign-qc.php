<?php 
  ...
  if($roleId == 1){
    ...
    $qcCount->execute();
    $tasks = $qcCount->fetchAll(PDO::FETCH_ASSOC);
    $countQc = count($tasks);
    ...
    foreach ($assign as $value) {
      ...
      $qcCount = $conn->prepare("SELECT * FROM `tasks` Where `status` = 'ready_for_qc' AND `project_id` = ?");
      $qcCount->execute([$value['project_id']]);
      $tasks[] = $qcCount->fetchAll(PDO::FETCH_ASSOC);
      ...
      $vetorCountt = $conn->prepare("SELECT * FROM `tasks` Where `vector_status` = 'pending' AND `project_id` = ?");
      $vetorCountt->execute([$value['project_id']]);
      $vetorCount[] = $vetorCountt->fetchAll(PDO::FETCH_ASSOC);
    }
  }
  ...
  <div class="container-flude pt-5 p-2">
    <div class="container-flude px-5">
      <div class="d-flex justify-content-between" style="font-size: 25px;">
        <p class="fw-bold page_heading">Assign Qc</p>
